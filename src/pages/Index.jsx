        ...
        <Flex overflow="hidden" direction={{ base: "column", md: "row" }}>
          ...
          <Box flex="1" bg="gray.100" p={4}>
            <Box flex="1" bg="gray.100" p={4}>
              ...
              <Flex h="20%" alignItems="center" justifyContent="space-between" p={4} borderTopWidth="1px" position="absolute" bottom="0" width="100%">
                <Button onClick={() => setActiveStep((prev) => (prev > 1 ? prev - 1 : prev))} isDisabled={activeStep === 1}>
                  Previous
                </Button>
                {activeStep < maxSteps && <Button onClick={() => setActiveStep((prev) => (prev < maxSteps ? prev + 1 : prev))}>Next</Button>}
                {activeStep === maxSteps && (
                  <Button colorScheme="blue" onClick={() => { /* Download logic here */ }}>
                    Download
                  </Button>
                )}
              </Flex>
